...
// Forward-declared so we don't pull splitscreen.hpp into this header. The
// inline helpers below resolve to "nearest Link" so NPC look-at / distance /
// angle queries automatically pick whichever Link is closer to the asker.
namespace dusk_ss { fopAc_ac_c* GetNearestPlayerToActor(const fopAc_ac_c*); }
#define _FOP_PLAYER_FOR(actor) (::dusk_ss::GetNearestPlayerToActor(actor))
#else
